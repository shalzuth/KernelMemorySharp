...
#define IOCTL_CALLBACK CTL_CODE(FILE_DEVICE_UNKNOWN, 0x0304, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define IOCTL_RPM CTL_CODE(FILE_DEVICE_UNKNOWN, 0x0305, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define IOCTL_WPM CTL_CODE(FILE_DEVICE_UNKNOWN, 0x0306, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
//#define IOCTL_UNLOAD CTL_CODE(FILE_DEVICE_UNKNOWN, 0x0307, METHOD_NEITHER, FILE_SPECIAL_ACCESS)
...
NTSTATUS IoCtlCallback(PDEVICE_OBJECT a, PIRP Irp)
{
	...
	else if (ControlCode == IOCTL_GET_PROC || ControlCode == IOCTL_GET_MODULE)
	{
		//DbgPrintEx(0, 0, "%wZ get process or module\n", &drv);
		PUINT64 Request = (PUINT64)Irp->AssociatedIrp.SystemBuffer;
		*Request = ControlCode == IOCTL_GET_PROC ? (UINT64)ProcessId : (UINT64)ImageBase;
		BytesIO = sizeof(*Request);
	}
